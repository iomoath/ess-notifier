...
A simple security notable scanner for Splunk ESS SIEM (Enterprise Security). The purpose of this tool is to send Slack & email notifications when a new security notable event is triggered on Splunk ESS.
...
After the scan is complete, you can run the follwoing command to send pending notifications if any:
```
python3 main.py -e
```
